 ...
 * @author lopeznr1
 */
class HideShowInteriorAction extends PopAction<Dem>
{
	// Ref vars
	...
	public HideShowInteriorAction(DemManager aManager, String aItemLabelStr)
	{
		refManager = aManager;
	...
	public void executeAction(List<Dem> aItemL)
	{
		...
		for (Dem aItem : aItemL)
			isAllShown &= refManager.getIsVisibleInterior(aItem) == true;

		// Update the tracks visibility based on whether they are all shown
		boolean tmpBool = isAllShown == false;
		refManager.setIsVisibleInterior(aItemL, tmpBool);
	}
