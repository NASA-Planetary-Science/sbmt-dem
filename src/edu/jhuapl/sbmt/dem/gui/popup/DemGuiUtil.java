...
public class DemGuiUtil
{
	...
	public static double getUnifiedOpacityFor(DemManager aItemManager, Collection<Dem> aItemC)
	{
		...
		for (Dem aItem : aItemC)
		{
			...
			isSameValue &= Objects.equal(tmpOpacity, retOpacity);
		}
	...
	public static double getUnifiedRadialOffsetFor(DemManager aItemManager, Collection<Dem> aItemC)
	{
		...
		for (Dem aItem : aItemC)
		{
			...
			isSameValue &= Objects.equal(tmpRadialOffset, retRadialOffset);
		}
	...
	/**
	 * Utility method that returns the "unified" isViewBadData for the provided
	 * list of {@link Dem}s.
	 * <p>
	 * If all items do not have the same value then null will be returned.
	 */
	public static Boolean getUnifiedViewBadData(DemManager aItemManager, Collection<Dem> aItemC)
	{
		if (aItemC.size() == 0)
			return null;
		...
		// Retrieve the opacity of first item
		Dem tmpItem = aItemC.iterator().next();
		boolean retViewBadData = aItemManager.getViewBadData(tmpItem);

		// Ensure all items have the same value
		boolean isSameValue = true;
		for (Dem aItem : aItemC)
		{
			boolean tmpViewBadData = aItemManager.getViewBadData(aItem);
			isSameValue &= Objects.equal(tmpViewBadData, retViewBadData);
		}

		if (isSameValue == false)
			return null;

		return retViewBadData;
	}

	/**
	 * Utility method that returns true if there is one DEM specified and it is
	 * visible.
	 */
